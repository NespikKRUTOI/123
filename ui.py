...
class Button:
    ...
    def draw(self, surface):
        ...
        pygame.draw.rect(surface, (0, 0, 0), self.rect, border_radius=15)

    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and self.rect.collidepoint(event.pos):
            self.sound.play()
            self.color = (220, 220, 220)  # Change color on click
            return True

        if event.type == pygame.MOUSEBUTTONUP:
            self.color = self.original_color  # Reset color on release
...
class Slider:
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN and (
            ...
            self.circle_x = max(self.rect.x, min(event.pos[0], self.rect.right))
